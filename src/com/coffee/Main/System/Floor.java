...
public class Floor {
	...
	private void getCombinations() {
		...
		try {
			fileReader = new InputStreamReader(getClass().getResource("/com/coffee/res/Templates/Floor.txt").openStream());
		} catch (IOException e) { }
	...
	public Floor(BufferedImage Compose) {
		getCombinations();
		IMAGE_TILE = Compose;
		if(IMAGE_TILE == null) {
	...
	private BufferedImage getTile(BufferedImage[] chunks, int index) {
		...
		for(int y = 0; y < 3; y++) {
			for(int x = 0; x < 2; x++) {
				if(Combination_System[index].length-1 < (x+y*2) || Combination_System[index][x+y*2] == -1) continue;
